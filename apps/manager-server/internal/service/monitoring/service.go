...
func buildAccountStats(stats []store.AccountModelStat, prices map[string]store.ModelPrice) []AccountStatRow {
	grouped := map[string]*accountStatAccumulator{}
	for _, stat := range stats {
		originalProvider := stat.AuthProviderSnapshot
		stat.AuthProviderSnapshot = effectiveProviderSnapshot(
			stat.AuthProviderSnapshot,
			stat.Model,
			stat.BillingModel,
...
func effectiveProviderSnapshot(provider string, modelNames ...string) string {
	...
	for _, modelName := range modelNames {
		if inferred := inferProviderFromModelName(modelName); inferred != "" && shouldPreferInferredProvider(provider, inferred) {
			return inferred
		}
	}
	return provider
...
func shouldPreferInferredProvider(provider, inferred string) bool {
	normalizedProvider := strings.ToLower(strings.TrimSpace(provider))
	if normalizedProvider == "" || normalizedProvider == "apikey" || normalizedProvider == "api-key" || normalizedProvider == "api_key" {
		return true
	}

	switch inferred {
	case "zhipu", "mimo", "minimax", "deepseek", "kimi", "qwen", "xai", "gemini":
		switch normalizedProvider {
		case "claude", "anthropic", "openai", "codex", "apikey", "api-key", "api_key":
			return true
		}
	}
	return false
...
func isMaskedAccountSnapshot(value string) bool {
