...
func sameAnalyticsFilter(left store.AnalyticsFilter, right store.AnalyticsFilter) bool {
	return left.FromMS == right.FromMS &&
		...
		left.MinLatencyMS == right.MinLatencyMS &&
		left.CacheStatus == right.CacheStatus &&
		...
		sameStringSlice(left.AuthFiles, right.AuthFiles) &&
		...
		sameStringSlice(left.ProjectIDs, right.ProjectIDs) &&
		sameStringSlice(left.RequestTypes, right.RequestTypes)
}
...
func (s *Service) filterOptions(ctx context.Context, optionFilter store.AnalyticsFilter, prices map[string]store.ModelPrice) (*FilterOptions, error) {
	...
	return buildFilterOptionsFromStats(accountStats, apiKeyStats, channelStats, modelStats, optionValues, prices), nil
}
