...
func TestAnalyticsAccountAndAPIKeyStatsUseFullFilteredScope(t *testing.T) {
	...
	totalAccountCalls := int64(0)
	totalAccountFailures := int64(0)
	totalAccountTokens := int64(0)
	totalAccountModels := 0
	for _, row := range resp.AccountStats {
		totalAccountCalls += row.Calls
		totalAccountFailures += row.FailureCalls
		totalAccountTokens += row.TotalTokens
		totalAccountModels += len(row.Models)
	}
	if totalAccountCalls != 3 || totalAccountFailures != 1 || totalAccountTokens != 43 {
		t.Fatalf("account stats totals = calls %d failures %d tokens %d; rows=%#v", totalAccountCalls, totalAccountFailures, totalAccountTokens, resp.AccountStats)
	}
	if totalAccountModels != 2 {
		t.Fatalf("account model stats count = %d; rows=%#v", totalAccountModels, resp.AccountStats)
	}
